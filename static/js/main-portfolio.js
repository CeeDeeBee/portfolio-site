			...
			for (let i = 0; i < writerPageData.length; i++) {
				...
				if (writerPageData[i].id === essayId) {
					...
					for (let j = 0; j < writerPageData[i].content.length; j++) {
						essayContent.append(
							$("<p></p>").text(writerPageData[i].content[j])
						);
					}
				}
			}
...
function aboutPageInit() {
	...
	if (!isAboutPageInit) {
		$.getJSON("/static/data.json", function (data) {
			...
			$(aboutContainer.find("a")[0])
				.prop({
					href: "/static/" + aboutPageData.resume,
					target: "_blank",
				})
				.text("Resume")
				.addClass("resume-link");
			...
			for (let i = 0; i < aboutPageData.socialLinks.length; i++) {
				...
				$(aboutDiv.find("a")[i]).append(
					$("<img>")
						.prop("src", "/static/images/" + aboutPageData.socialLinks[i].img)
						.addClass("about-img")
				);
			}
		});
